...
export default function Sidebar({
  ...
  const { clients, selectedClient, selectedClientId, setSelectedClientId, teams, tasks, assignableProfiles } = useData();
        ...
        {assignableProfiles.map((p) => {
          ...
          return (
            <button key={p.id} className={`team-filter-item${on ? '' : ' off'}`} onClick={() => toggleAssignee(p.id)}>
              <span className="avatar" style={{ width: 18, height: 18, fontSize: 8.5 }}>{initials(p.name)}</span>
