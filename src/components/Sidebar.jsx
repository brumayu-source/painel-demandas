...
import { IconBoard, IconList, IconChart, IconGear, IconChevron, IconSignOut, IconClose, IconInbox, IconArchive, IconContent } from './icons.jsx';
...
export default function Sidebar({
  ...
  // conteúdo (social media) é um ambiente separado das demandas — não entra
  // nas contagens de time/responsável/cliente/categoria/arquivadas daqui
  const demandas = tasks.filter((t) => !t.content_format);

  const activeCounts = {};
  teams.forEach((t) => { activeCounts[t.id] = demandas.filter((x) => x.team_id === t.id && x.stage !== 'done').length; });

  const assigneeCounts = {};
  assignableProfiles.forEach((p) => { assigneeCounts[p.id] = demandas.filter((x) => x.assignee_id === p.id && x.stage !== 'done').length; });

  const clientCounts = {};
  clients.forEach((c) => { clientCounts[c.id] = demandas.filter((x) => x.client_id === c.id && x.stage !== 'done').length; });

  const categoryCounts = {};
  categories.forEach((c) => { categoryCounts[c.id] = demandas.filter((x) => x.category_id === c.id && x.stage !== 'done').length; });

  const clientById = (id) => clients.find((c) => c.id === id);

  const solicitacoesCount = demandas.filter((t) => t.stage === 'solicitado').length;
  const archivedCount = demandas.filter((t) => t.archived).length;
  const conteudoCount = tasks.filter((t) => t.content_format && t.stage === 'aprovacao' && !t.archived).length;
    ...
    <div id="sidebar" className={open ? 'mobile-open' : ''}>
      ...
      <nav className="side-nav">
        <NavItem active={view === 'board'} onClick={() => { setView('board'); onClose?.(); }} icon={<IconBoard />} label="Quadro"
          count={demandas.filter((t) => t.stage !== 'done').length} />
        <NavItem active={view === 'list'} onClick={() => { setView('list'); onClose?.(); }} icon={<IconList />} label="Lista" count={demandas.length} />
        <NavItem active={view === 'overview'} onClick={() => { setView('overview'); onClose?.(); }} icon={<IconChart />} label="Painel" />
        <NavItem active={view === 'conteudo'} onClick={() => { setView('conteudo'); onClose?.(); }} icon={<IconContent />} label="Conteúdo"
          count={conteudoCount} />
        ...
        {assignableProfiles.map((p) => {
          ...
          return (
            <button key={p.id} className={`team-filter-item${on ? '' : ' off'}`} onClick={() => toggleAssignee(p.id)}>
              <span className="avatar" style={{ width: 18, height: 18, fontSize: 8.5, background: personColorVar(p), color: '#fff' }}>{initials(p.name)}</span>
