...
import { STAGES, stageLabel, teamColorVar, categoryColorVar, personColorVar, fmtDateShort, todayStr } from '../lib/format.js';
import TaskModal from '../components/TaskModal.jsx';
import ContentModal from '../components/ContentModal.jsx';

export default function ListView({ activeTeamIds, activeAssigneeIds, activeClientIds, activeCategoryIds, search, openNewSignal, onConsumeNewSignal }) {
  const { tasks, teams, clients, categories, assignableProfiles, selectedClientId, canWrite } = useData();
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      else if (sortCol === 'category') { va = categoryById(a.category_id)?.name || ''; vb = categoryById(b.category_id)?.name || ''; }
  ...
  const cols = [
    ...
    ['team', 'Time'], ['category', 'Categoria'], ['assignee', 'Responsável'],
  ...
  if (!sorted.length) {
    return (
      <div className="empty-state">
        ...
        {modalTask !== undefined && (modalTask?.content_format
          ? <ContentModal task={modalTask} onClose={() => setModalTask(undefined)} />
            ...
            return (
              <tr key={t.id}>
                ...
                <td data-label="Responsável">
                  {assignee ? (
                    <span className="team-tag">
                      <span className="team-dot" style={{ background: personColorVar(assignee) }} />
                      {assignee.name}
                    </span>
                  ) : <span style={{ color: 'var(--text-muted)' }}>—</span>}
                </td>
