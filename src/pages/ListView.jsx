...
export default function ListView({ activeTeamIds, search, openNewSignal, onConsumeNewSignal }) {
  const { tasks, teams, assignableProfiles, canWrite } = useData();
  ...
  const cols = [
    ['title', 'Tarefa'], ['team', 'Time'], ['assignee', 'Responsável'],
          ...
          {sorted.map((t) => {
            ...
            return (
              <tr key={t.id}>
                ...
                <td><span className={`status-badge ${t.stage}`}>{stageLabel(t.stage)}</span></td>
                <td className="tnum" style={overdue ? { color: 'var(--critical)', fontWeight: 600 } : undefined}>
                  {t.due_date ? fmtDateShort(t.due_date) : '—'}
                </td>
                <td>{t.priority === 'alta' ? 'Alta' : t.priority === 'baixa' ? 'Baixa' : 'Normal'}</td>
