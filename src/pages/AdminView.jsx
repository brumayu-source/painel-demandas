...
import { PERSON_COLOR_COUNT, PERSON_COLOR_NAMES } from '../lib/format.js';
...
function UsersTab() {
  const { clients, reloadProfiles } = useData();
  ...
  const reload = useCallback(async () => {
    ...
    setLoading(false);
    // mantém o resto do app (barra lateral, seletor de responsável, avatares
    // nos cards) sincronizado assim que algo muda aqui — sem isso, só
    // atualizava depois de recarregar a página inteira
    reloadProfiles();
  }, [toast, reloadProfiles]);
  ...
  async function setName(profileId, name) {
    const { error } = await supabase.from('profiles').update({ name }).eq('id', profileId);
    if (error) { toast('Não foi possível renomear: ' + error.message); return; }
    reload();
  }

  async function setColorSlot(profileId, slot, current) {
    const nextSlot = current === slot ? null : slot; // clicar na cor já escolhida volta pro automático
    const { error } = await supabase.from('profiles').update({ color_slot: nextSlot }).eq('id', profileId);
    if (error) { toast('Não foi possível mudar a cor: ' + error.message); return; }
    reload();
  }
  ...
  return (
    <div>
      <div className="banner">
        ...
        clientes na tabela depois de criada. Editor e viewer só enxergam os clientes marcados pra eles aqui;
        só admin vê todos automaticamente.
      </div>
              ...
              <tr key={p.id}>
                <td data-label="Nome">
                  <input
                    ...
                    defaultValue={p.name}
                    placeholder="Nome da pessoa"
                    onBlur={(e) => { const v = e.target.value.trim(); if (v && v !== p.name) setName(p.id, v); }}
                  />
                  {p.id === currentUser?.id && <span style={{ color: 'var(--text-muted)', fontSize: 11 }}> (você)</span>}
                </td>
                ...
                <td data-label="Cor">
                  {p.role === 'viewer' ? (
                    ...
                    <div className="color-swatch-list">
                      {Array.from({ length: PERSON_COLOR_COUNT }, (_, slot) => (
                        <button
                          key={slot}
                          type="button"
                          className={`color-swatch${p.color_slot === slot ? ' on' : ''}`}
                          style={{ background: `var(--person-${slot + 1})` }}
                          title={PERSON_COLOR_NAMES[slot]}
                          aria-label={`Cor ${PERSON_COLOR_NAMES[slot]}`}
                          onClick={() => setColorSlot(p.id, slot, p.color_slot)}
                        />
                      ))}
                    </div>
                  )}
                </td>
                <td data-label="Clientes com acesso">
                  {p.role === 'admin' ? (
                    <span style={{ color: 'var(--text-muted)', fontSize: 12 }}>vê todos (admin)</span>
                  ) : (
...
function CategoriesTab() {
  const { categories, createCategory, renameCategory, removeCategory } = useData();
  const toast = useToast();
  const [newName, setNewName] = useState('');
  ...
  return (
    <div className="settings-list" style={{ maxWidth: 480 }}>
      <p className="sub" style={{ color: 'var(--text-secondary)', fontSize: 13, marginTop: 0 }}>
        Categorias globais, disponíveis pra todos os clientes.
      </p>
      {categories.map((c) => (
        <div className="settings-row" key={c.id}>
          <input
      ...
      {categories.length === 0 && <div className="empty-mini">Nenhuma categoria ainda.</div>}
      <div className="add-inline">
        <input type="text" placeholder="Nome da nova categoria" value={newName} onChange={(e) => setNewName(e.target.value)} />
        <button className="btn btn-ghost" onClick={handleAdd}>+ Adicionar</button>
      </div>
    </div>
  );
