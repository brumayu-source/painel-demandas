...
export function DataProvider({ children }) {
  ...
  // pick a default selected client once the list loads
  useEffect(() => {
    if (clientsLoading || !clients.length) return;
    const stillValid = clients.some((c) => c.id === selectedClientId);
  ...
  const reloadTeams = useCallback(async (clientId) => {
    ...
    const { data, error } = await supabase.from('teams').select('*').eq('client_id', clientId).order('slot');
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'teams', filter: `client_id=eq.${selectedClientId}` },
        () => reloadTeams(selectedClientId)
      )
      .subscribe();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'tasks', filter: `client_id=eq.${selectedClientId}` },
