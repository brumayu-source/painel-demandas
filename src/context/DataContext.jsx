...
export function DataProvider({ children }) {
  ...
  // ---------- anexos de todo mundo (agrupados por task_id) ----------
  // usado pela grade/calendário de Conteúdo pra mostrar miniatura sem cada
  // card ter que buscar o próprio anexo. Tem assinatura em tempo real
  // própria (não só quando `tasks` muda) porque subir um anexo não mexe na
  // tabela tasks — sem isso, a miniatura só apareceria depois de alguma
  // outra mudança de demanda "por acaso" disparar um refresh
  const [attachmentsByTask, setAttachmentsByTask] = useState({});
  const reloadAttachmentsByTask = useCallback(async (clientId) => {
    if (!clientId) { setAttachmentsByTask({}); return; }
    let q = supabase.from('task_attachments').select('*').order('created_at');
    if (clientId !== 'ALL') q = q.eq('client_id', clientId);
    const { data, error } = await q;
    if (error) return;
    const grouped = {};
  ...
  useEffect(() => {
    if (!selectedClientId) { setAttachmentsByTask({}); return; }
    reloadAttachmentsByTask(selectedClientId);
    const filter = selectedClientId === 'ALL' ? undefined : `client_id=eq.${selectedClientId}`;
    const channel = supabase
      .channel('task-attachments-changes-' + selectedClientId)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'task_attachments', ...(filter ? { filter } : {}) },
        () => reloadAttachmentsByTask(selectedClientId)
      )
      .subscribe();
    return () => supabase.removeChannel(channel);
  }, [selectedClientId, reloadAttachmentsByTask]);
  ...
  // url (blob local) pra exibir a imagem de um anexo em tela — o bucket é
  // privado, então precisa baixar o arquivo pra conseguir mostrar; cacheia
  // por anexo pra não baixar de novo toda hora que um card re-renderiza
  const attachmentUrlCache = useRef(new Map());
  const getAttachmentUrl = useCallback(async (att) => {
    if (!att) return null;
    if (attachmentUrlCache.current.has(att.id)) return attachmentUrlCache.current.get(att.id);
    const { data, error } = await supabase.storage.from('attachments').download(att.storage_path);
    if (error || !data) return null;
    const url = URL.createObjectURL(data);
    attachmentUrlCache.current.set(att.id, url);
    return url;
  }, []);

  // ---------- conteúdo (Social Media): histórico de aprovação ----------
  const listFeedback = useCallback(async (taskId) => {
    const { data, error } = await supabase
      .from('task_feedback')
      .select('*')
      .eq('task_id', taskId)
  ...
  // aprova ou pede ajuste numa demanda "Em Aprovação" — funciona pro cliente
  // (viewer) também, mesmo ele não tendo permissão de editar tasks; a regra
  // de negócio toda mora na função do banco (submit_content_review)
  const submitContentReview = useCallback(
    async (taskId, action, message) => {
      const { data, error } = await supabase.rpc('submit_content_review', {
        p_task_id: taskId,
        p_action: action,
