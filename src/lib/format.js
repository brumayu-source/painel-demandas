...
// status de aprovação de um conteúdo — cor e rótulo específicos pro
// ambiente de Conteúdo (diferente do status genérico de demanda): verde só
// quando o cliente aprovou de fato, vermelho quando pediu ajuste, âmbar
// enquanto está esperando a decisão do cliente. "Em Revisão" também aparece
// como "Ajuste solicitado" porque é pra onde a demanda volta automaticamente
// quando o cliente pede ajuste (função submit_content_review) — se um dia
// usarem esse status pra revisão interna antes de mandar pro cliente, vale
// revisar esse rótulo.
export function contentStatusLabel(stage) {
  if (stage === 'aprovacao') return 'Aguardando aprovação';
  if (stage === 'review') return 'Ajuste solicitado';
  if (stage === 'done') return 'Aprovado';
  return stageLabel(stage);
}

export function contentStatusColorVar(stage) {
  if (stage === 'aprovacao') return 'var(--warning)';
  if (stage === 'review') return 'var(--critical)';
